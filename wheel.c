...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    char ** new_words;
    int first_index = 0;
    while(words[first_index] != NULL){
      if (strlen(words[first_index]) == len){
          length++;
      }
      first_index++;
    }
    ...
    *words_remaining = length;
    new_words = malloc(sizeof(char*)*(length+1));
    while(words[second_index] != NULL){
      if (strlen(words[second_index]) == len){
        new_words[index] = words[second_index];
        index++;
      }
      second_index++;
    }
    new_words[length] = NULL;
    return new_words;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    while (words_remaining==0){
      ...
      fgets(str, 60, stdin);
      int i = 0;
      i = strtol(str, &extras, 10);
      if (i==0){
        printf("There are no words of that length.\n");
        printf("Length of words to use? ");
      }
      else {
        word_list = prune_word_list(words, i, &words_remaining);
        *len = i;
      }
